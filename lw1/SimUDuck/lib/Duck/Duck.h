...
public:
	...
	void Fly()
	{
		if ((m_flyBehavior->GetCountFly() + 1) % 2 == 0 && m_flyBehavior->GetCountFly() != 0)//тест который проверятет условие и метод добавить чтобы с нулём не сравнивать
		{
			Quack();
		}
		m_flyBehavior->SetCountFly();
		m_flyBehavior->Fly();
	}
